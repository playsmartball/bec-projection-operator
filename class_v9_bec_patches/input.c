...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 8.c) BEC condensate (Landau-Ginzburg model) */
  class_call(parser_read_string(pfc,"has_bec",&string1,&flag1,errmsg),
             errmsg,errmsg);
  if (flag1 == _TRUE_) {
  ...
  class_read_double("bec_lambda",pba->bec_lambda);
  class_read_double("bec_g",pba->bec_g);
  class_read_double("bec_alpha",pba->bec_alpha);
  class_read_double("bec_phi_ini",pba->bec_phi_ini);
  class_read_double("bec_dphi_ini",pba->bec_dphi_ini);

  /* V9 BEC Crust: minimal vacuum substrate model
   * 
   * When enabled:
   * - Replaces Λ with Ω_BEC(z) = Ω_BEC_0 × (1+z)^n_bec
   * - n_bec = 0 recovers exact ΛCDM
   * - Ω_BEC_0 is fixed by closure (uses Omega0_lambda internally)
   * - No clustering (perturbations treated like Λ)
   */
  class_call(parser_read_string(pfc,"bec_crust",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)) {
      pba->has_bec_crust = _TRUE_;
      /* When BEC crust is enabled, we use Omega0_lambda to store Ω_BEC_0
       * which is computed from closure. The has_lambda flag remains TRUE
       * so that perturbations are treated like Λ (no clustering). */
    }
    else {
      pba->has_bec_crust = _FALSE_;
    }
  }
...
int input_default_params(struct background *pba,
  ...
  /* BEC condensate defaults (Landau-Ginzburg model) */
  pba->has_bec = _FALSE_;
  pba->bec_lambda = 2.0;      /* self-interaction coefficient */
  pba->bec_g = -0.5;          /* matter coupling coefficient */
  pba->bec_alpha = 0.75;      /* stabilizing quartic coefficient */
  pba->bec_phi_ini = 0.99;    /* initial field value */
  pba->bec_dphi_ini = 0.0;    /* initial derivative */
  
  /* V9 BEC Crust defaults (minimal vacuum substrate model) */
  pba->has_bec_crust = _FALSE_;
  pba->n_bec = 0.0;           /* n_bec = 0 recovers exact ΛCDM */
