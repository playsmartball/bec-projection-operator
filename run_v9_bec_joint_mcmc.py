#!/usr/bin/env python3
"""
V9 BEC Crust Joint MCMC Analysis: Planck + BAO + SNe

This script runs MCMC sampling to constrain n_bec using:
- Planck 2018 CMB (TT, TE, EE + lowl)
- BAO measurements (BOSS DR12, 6dF, SDSS MGS)
...
"Does adding BAO + SNe tighten the constraint on n_bec?"
"Is n_bec ≈ 0.5 still allowed with joint constraints?"
"""
...
# Output directory
OUTPUT_DIR = '/Users/hodge/Desktop/sanity_check/fracos/phi/v9_bec_joint_mcmc_results'
os.makedirs(OUTPUT_DIR, exist_ok=True)

# MCMC Configuration with joint likelihoods
info = {
    "likelihood": {
        # Planck 2018 CMB
    ...
    "output": os.path.join(OUTPUT_DIR, "v9_bec_joint"),
    "force": True,
}
...
def run_quick_test():
    ...
    print("=" * 60)
    print("Quick Test: Verifying CLASS + Joint likelihood setup")
    print("=" * 60)
    ...
    # Test BAO-relevant quantities
    print(f"  D_V(z=0.106) = {cosmo.get_background()['comov. dist.'][0]:.2f} Mpc (for 6dF)")
    
    cosmo.struct_cleanup()
    
    # Test Cobaya likelihood loading
    try:
    ...
    print(f"\n✓ Quick test passed - ready for joint MCMC")
    return True


def run_mcmc():
    """Run the full joint MCMC analysis."""
    print("=" * 60)
    print("V9 BEC Crust Joint MCMC Analysis")
    print("Planck + BAO + SNe")
    print("=" * 60)
    print(f"\nOutput directory: {OUTPUT_DIR}")
    print(f"Parameter to constrain: n_bec")
    print(f"Prior: n_bec ∈ [-1, 2]")
    print(f"\nLikelihoods:")
    print(f"  - Planck 2018 highl plik.TTTEEE_lite")
    print(f"  - Planck 2018 lowl.TT")
    print(f"  - BAO: 6dF, SDSS MGS, BOSS DR12")
    print(f"  - SNe: Pantheon")
    ...
    try:
        ...
        if hasattr(sampler, 'products'):
            ...
            if 'sample' in products:
                ...
                print(f"\nn_bec constraints (Planck + BAO + SNe):")
                ...
                sigma_from_00 = abs(0.0 - mean) / std
                print(f"\n  n_bec = 0.0 (ΛCDM) is {sigma_from_00:.1f}σ from mean")
                print(f"  n_bec = 0.5 (V9) is {sigma_from_05:.1f}σ from mean")
                
                if sigma_from_05 < 2:
                    print(f"  → V9 prediction (n_bec ≈ 0.5) is ALLOWED by joint data")
                else:
                    print(f"  → V9 prediction (n_bec ≈ 0.5) is DISFAVORED by joint data")
                
                # Compare with Planck-only result
                print(f"\n  Comparison with Planck-only:")
                print(f"  Planck-only: n_bec = 0.18 ± 0.40")
                print(f"  Joint:       n_bec = {mean:.2f} ± {std:.2f}")
                
                improvement = 0.40 / std if std > 0 else 1.0
                print(f"  Constraint improvement: {improvement:.1f}x tighter")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='V9 BEC Crust Joint MCMC Analysis')
