...
def run_linear_periodic(Lx=128.0, Nx=128, Lz=128.0, Nz=128, B0=1.0, tau=1.0,
                         rho_perp=20.0, rho_par=5.0, eps=0.0,
                         tmax=10.0, tstep=1e-3, amp=1e-6, m=1, n=1, eta=0.0, csv_path=None):
    ...
    zbasis = de.Fourier(coords['z'], size=Nz, bounds=(0.0, Lz), dealias=3/2, dtype=dist.dtype)
    ...
    problem = de.IVP([vx, vy, bx, by, wx, wy], namespace=locals())
    ...
    problem.add_equation("dt(bx) - tau*B0*wx - eta*(d_xx(bx) + d_zz(bx)) = 0")
    problem.add_equation("dt(by) - tau*B0*wy - eta*(d_xx(by) + d_zz(by)) = 0")
    ...
    sz = np.sin(2.0 * np.pi * n * z / Lz)
    cz = np.cos(2.0 * np.pi * n * z / Lz)

    vx['g'] = amp * sx * sz
    vy['g'] = 0.0
    bx['g'] = 0.0
    by['g'] = 0.0
    wx['g'] = amp * sx * (2.0 * np.pi * n / Lz) * cz
    wy['g'] = 0.0
    ...
    if csv_path is None:
        ...
        csv_path = os.path.join(outdir, f'lin_periodic_nx{Nx}_m{m}_n{n}_eta{eta}_{stamp}.csv')
    else:
    ...
    while solver.proceed:
        ...
        if eta == 0.0:
            ...
            # Resistive boundary power diagnostics (periodic z: expect total ≈ 0)
...
def main():
    p = argparse.ArgumentParser(description="Phase-6 linear periodic IVP harness with energy diagnostics")
    ...
    res = run_linear_periodic(Lx=args.Lx, Nx=args.Nx, Lz=args.Lz, Nz=args.Nz, B0=args.B0, tau=args.tau,
